  ...
  <div class="index">
    ...
    <div class="inquiry_view">
      <ul class="inquiry_tab clearfix">
        <li class="active">
          <a @click="reset()">已报价询盘<span>({{Quoted}})</span></a>
        </li>
        <li>
          <router-link to="/inquiry/wait">待报价询盘<span>({{wait_Quoted}})</span></router-link>
        </li>
      </ul>
    </div>
      ...
      <div class="inquiry">
        <div class="inquiry_filter clearfix">
          ...
          <div class="pull-right key_search clearfix">
            <input type="text" class="search_key pull-left" no="search_key" @click="showpod()" @keyup="getSearchPod()" placeholder="请输入目的港" @keyup.enter="search" v-model="search_pod"/>
            ...
            <div class="allPodWithRoute" v-if="pod_show == true">
              ...
              <div class="area_ditail clearfix">
                <ul class="pull-left area">
                  <li v-for="(data,index) in area" @click="choosearea(index)" no="pod_item">{{data.routeName}}</li>
                </ul>
        ...
        <div class="inquiry_list" v-if="pages.totalnum != 0">
            ...
            <li v-for="(item,index) in jsondata" class="item">
              ...
              <div class="item_main clearfix">
                ...
                <div class="operate pull-right">
                  ...
                  <div class="edit_quotation" v-if="item.inquiryStatus > 1">
                    <router-link :to="{ name: 'quotation', params: { id: item.id }}" target="_blank">
                      <img style="transform:translateY(2px);margin-right: 5px;" src="../../assets/images/icon_bjd_chakan.png">
                      查看报价单
                    </router-link>
                    <a @click="download(item)">
                      <img style="margin-right:5px;" src="../../assets/images/icon_bjd_xiazai.png"/>
                      下载报价单
                    </a>
                  </div>
                </div>
...
<script>
  ...
  export default {
    ...
    created () {
      ...
      $self.condition.inquiryStatus = 2
